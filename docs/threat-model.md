...
| Popularity mistaken for safety | a top-ranked or user-content domain causes every path to be labeled safe | detection always runs; popularity is separate context; only exact HTTPS roots in a small reviewed platform registry can be low risk | The registry confirms domain identity, not future compromise or third-party linked content |
